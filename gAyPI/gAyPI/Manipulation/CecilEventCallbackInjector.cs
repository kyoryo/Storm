...
namespace gAyPI.Manipulation
{
    public class CecilEventCallbackInjector : Injector
    {
        ...
        public void Inject()
        {
            ...
            var popIns = returnName.Equals("System.Void") ? ins : processor.Create(OpCodes.Pop);

            processor.InsertBefore(ins, processor.Create(OpCodes.Call, injectee.IsStatic ? staticImport : instancedImport));
            if (!returnName.Equals("System.Void"))
            {
            ...
            processor.InsertBefore(ins, processor.Create(OpCodes.Brfalse_S, popIns));
            
            if (returnName.Equals("System.Int32") || returnName.Equals("System.Boolean"))
            {
            ...
            if (!returnName.Equals("System.Void"))
            {
                processor.InsertBefore(ins, popIns);
            }
